...
def FFRI(X, K, C, alpha, L, M, noise_threshold, save_path=None, label=None):
    best_nmi = -1
    best_nmi_iter = -1
...
def FGI(X, K, C, L, M, noise_threshold, save_path=None, label=None):
    best_nmi = -1
    best_nmi_iter = -1
    X = X - np.mean(X, axis=0)
    Y = extract_supporting_points(samples=X, M=M, noise_threshold=noise_threshold)
        ...
        if method_opt == 'FRI':
            ...
            for K in K_list:
                for alpha in alpha_list:
                    ...
                    FFRI(data, K, C, alpha, L, M, noise_threshold, save_path=rec_dn_dir + para_str + '.txt',
                         label=label)
        if method_opt == 'GI':
            L = 30
            for K in K_list:
                para_str = 'K({:02})'.format(K)
                FGI(data, K, C, L, M, noise_threshold, save_path=rec_dn_dir + para_str + '.txt', label=label)
        if method_opt == 'PIC':
            L = 180
            for K in K_list:
                para_str = 'K({:02})'.format(K)
