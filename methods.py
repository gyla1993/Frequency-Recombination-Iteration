...
def FRI(X, K, C, alpha, L, save_path=None, label=None):
    best_nmi = -1
    best_nmi_iter = -1
...
def GI(X, K, C, L, save_path=None, label=None):
    best_nmi = -1
    best_nmi_iter = -1
    X = X - np.mean(X, axis=0)
    E = calc_dist(X, X)
        ...
        if method_opt == 'FRI':
            ...
            for K in K_list:
                for alpha in alpha_list:
                    ...
                    FRI(data, K, C, alpha, L, save_path=rec_dn_dir + para_str + '.txt', label=label)
        if method_opt == 'GI':
            L = 30
            for K in K_list:
                para_str = 'K({:02})'.format(K)
                GI(data, K, C, L, save_path=rec_dn_dir + para_str + '.txt', label=label)
